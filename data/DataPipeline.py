...
class TFRecords(Pipeline_Interface):
    ...
    def populate_training_data(self) -> None:
        ...
        for index, game in enumerate[dict[str, list[str]]](dataset['train']):
            ...
            tfrecord_path: str = f"training_data/{self.name}_{current_record:04}"
            with tf.io.TFRecordWriter(path=tfrecord_path) as writer:
                for tensor in data:
                    serialized_example = self.serialize_features_with_labels(tensor[0], tensor[1])
