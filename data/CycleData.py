...
class Cycle_Data(Pipeline_Interface):
    ...
    def train_model(self, model) -> None:
        ...
        for index, game in enumerate[dict[str, list[str]]](huggingface_data['train']):
            ...
            dataset = dataset.batch(batch_size=512, num_parallel_calls=tf.data.AUTOTUNE)
            dataset = dataset.prefetch(buffer_size=tf.data.AUTOTUNE)

            validation: tf.data.Dataset[tuple[tf.Tensor, tf.Tensor]] = dataset.skip(int(data_size*0.8)).take(int(data_size*0.2))
            ...
            plt.savefig(f"{self.directory}/plots/{iteration-1:06}.pdf", format='pdf', bbox_inches='tight')
